    ...
    <div class="p-6">
        @foreach ($schedule->course->students as $student)
            <div class="grid grid-cols-6 border-b group hover:bg-gray-50">
                <div class="col-span-4 py-2 flex space-x-2 items-center pl-4">
                    {{ $student->first_name }}
                    {{ $student->last_name }}
                </div>
                <div class="col-span-2 py-2 flex items-center justify-center">
                    @if ($schedule->students->contains($student))
                        <a wire:click="removeStudent({{$student}})"
                    ...
                    @else
                        <a wire:click="addStudent({{$student}})"
