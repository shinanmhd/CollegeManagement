...
@section('content')
    <div class="flex flex-col">
        ...
        <div class="flex items-center justify-between bg-gray-50 border-b border-gray-100 p-4">
            ...
            <a href="{{ route('admin.student.create') }}" class="px-4 py-2 rounded-lg bg-green-100 text-green-900 hover:bg-green-500 hover:shadow-xl hover:text-white flex items-center space-x-2 transition-all">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v3m0 0v3m0-3h3m-3 0H9m12 0a9 9 0 11-18 0 9 9 0 0118 0z" />
                </svg>
                <span class="">New Student</span>
            </a>
        </div>
        ...
        @foreach ($students as $student)
            <div class="grid grid-cols-12 border-b border-l border-r hover:bg-gray-50 font-base">
                ...
                <div class="col-span-2 flex items-center p-4">{{ $student->first_name . " " . $student->last_name }}</div>
                <div class="col-span-2 flex items-center p-4">{{ $student->user->email }}</div>

                <div class="col-span-1 flex items-center grid grid-cols-6">
                    <div class="col-span-3 flex items-center p-4">{{ $student->gender == 'm' ? "Male" : "Female" }}</div>
                    <div class="col-span-3 flex items-center p-4">{{ $student->age }}</div>
                </div>

                <div class="col-span-2 flex items-center p-4">{{ $student->contact_address }}</div>
                <div class="col-span-2 flex items-center p-4">
                    @livewire('student.student-courses', ['student' => $student])
                </div>
                <div class="col-span-2 flex items-center justify-center space-x-2">
                    <a onclick="Livewire.emit('openModal', 'student.assign-course', {{ json_encode(["student" => $student]) }})"
                    ...
                    <a href="{{ route('admin.student.edit', $student->user->id) }}"
                    ...
                    <form method="POST" action="{{ route('admin.student.destroy', [ 'student'=> $student ]) }}" onSubmit="if(!confirm('Are you sure you want to delete this user?')){return false;}">
                        @csrf
