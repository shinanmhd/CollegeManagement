...
@section('content')
    <div class="flex flex-col">
        ...
        <div class="flex items-center justify-between bg-gray-50 border-b border-gray-100 p-4">
            <h1 class="text-3xl font-semibold">New subject</h1>
        ...
        {{--form--}}
        <form action="{{ route('admin.subject.store') }}" method="post" class="flex flex-col space-y-8 p-12 m-6 w-3/4 border rounded-lg mt-12">
            @csrf
                ...
                <div class="flex flex-col w-full">
                    ...
                    <select name="course_id" id="course_id">
                        ...
                        @foreach ($courses as $course)
                            <option value="{{ $course->id }}" {{ $toCourse == $course->id ? 'selected' : '' }}>{{ $course->name }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
