...
@section('content')
    <div class="flex flex-col">
        ...
        <div class="flex items-center justify-between bg-gray-50 border-b border-gray-100 p-4">
            <h1 class="text-3xl font-semibold">Edit Transaction</h1>
        ...
        {{--form--}}
        <form action="{{ route('admin.transaction.update', ['transaction' => $transaction]) }}" method="post" class="flex flex-col space-y-8 p-12 m-6 w-3/4 border rounded-lg mt-12">
            @csrf
            @method('patch')
            ...
            <div class="flex space-x-8">
                <div class="flex flex-col w-full">
                    ...
                    <input type="text" name="transaction_name" id="transaction_name" class="input-text" value="{{ old('transaction_name', $transaction->transaction_name) }}">
                </div>
            </div>
                ...
                <div class="flex flex-col w-full">
                    ...
                    <select name="student_id" id="student_id">
                        ...
                        @foreach ($students as $student)
                            <option value="{{ $student->id }}" {{ $transaction->student->id == $student->id ? 'selected' : '' }}>{{ $student->first_name }} {{ $student->last_name }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            ...
            <div class="flex space-x-8">
                <div class="flex flex-col w-full">
                    ...
                    <input type="date" name="transaction_date" id="transaction_date" class="input-text" value="{{ old('transaction_date', $transaction->transaction_date) }}">
                </div>
            </div>
